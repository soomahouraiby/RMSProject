                ...
                <div class="btn-group show ">
                    @if(isset($procedures))
                        @foreach($procedures as $procedure)
                            @if($procedure->report_id == $report->id_report)
                    <button  class="btn btn-sm btn-outline-secondary  mr-4 ml-4 button" >
                        <a href="{{route('PM_endFollowUp',$report->id_report)}}">انهاء المتابعة</a>
                    </button>
                            @endif
                            @break($report)
                        @endforeach
                    ...
                    @if(isset($reports))
                        @foreach($reports as $report)
                            ...
                            <div class="form-group raw mt-4  ">
                                <a class="text-center col-form-label mb-3"  href="{{route('PM_detailsReport',$report->id_report)}}" style="margin-right: 20%"> تفاصيل البلاغ</a>
                                @if($report -> type_report != 'مهرب')
                                    <a class="text-center col-form-label mb-3 "  href="{{route('PM_detailsReport',$report->id_report)}}" style="margin-right: 34%"> تفاصيل الدواء</a>
                                @endif
                            </div>
                        @endforeach
                    @endif
...
{{--                            @break($procedure)--}}
                ...
                @if($report->report_statuses == 'قيد المتابعة')
                     <form method="POST" action="{{route('PM_addProcedure',$report->id_report)}}">
                    @csrf
                        <div class="row pb-5">
                            <div class="form-group raw mt-4 col-lg-12 " style="display: flex; flex-wrap: wrap; ">
